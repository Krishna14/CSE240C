...
//  This class models a cache line in x86 processors
...
int main() 
{
    ...
    for(int i=0;i<NUM_ITER;i++) {
        ...
        srand(time(0));
        //  Swapping the nodes randomly to defeat the prefetcher
        ...
        //  Chase the pointer from one node to the next
        ...
        cout << "size: " << size * sizeof(CacheLookAlike) <<" Latency: " 
             << (double) ((endTime.tv_sec - startTime.tv_sec)*SECONDS_PER_NS + (endTime.tv_usec - startTime.tv_usec)*1000)/NUM_ACCESSES;
        cout<<"\n";
        free(Cache_DS);
        size *= 2;
    }
